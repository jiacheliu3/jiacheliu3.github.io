...
// float actually is number
// where is string?
// function different from verb?

var NIL = ks("");
var k0 = k(0, 0);
var k1 = k(0, 1);
var EC = [["\\","\\\\"],["\"","\\\""],["\n","\\n"],["\t","\\t"]];											// what is it?
var kt = [-9, -10, -11, 0, 99, 102, NaN, NaN, 107, 105, NaN, NaN, NaN];										// what is it?
var SP = k(1, " ".charCodeAt(0));																			// SPACE char
var NA = k(0, NaN);																							// does this exist in K? 

function k     (t, v)    { return { 't':t, 'v':v }; }														
function md    (x, y)    { var v={ t:4, k:sl(x,y), v:y };return v;}											// make dictionary, x is all keys and y all values
function ks    (x)       { return k(2, x); }																// parse to symbol
function asVerb(x, y, z) { return { t:8, v:x, l:y, r:z }; }													// l: left, r: right
function kl    (x)       { return x.length==1 ? x[0] : k(3,x);}												// first element or enlist, but why this design?
function kf    (x)       { return match(k(3,[]), x).v || match(k0, x).v; }									// 0 or empty list
function kb    (x)       { return x ? k1 : k0; }															// binary decison
function s     (x)       { return x.t == 3 && x.v.every(function(c) { return c.t == 1; }); }				// check if symbol
function kmod  (x, y)    { return x-y*Math.floor(x/y); }													// mod
function len   (x)       { return l(x).v.length; }															// length
function krange(x, f)    { 
	var r=[]; for(var z=0;z<x;z++) { r.push(f(z)); } 
	var v= k(3,r); 
	return v;
}				// apply f to int 0~x
																											// TODO: Test this 
function h2    (x)       { return (x.v+0x100).toString(16).substr(-2); }									// 2 digit num decimal to hex
function lget  (x, y)    { if(y<0||y>=len(x)) { throw new Error("length error."); } return x.v[y]; }		// list get at index
function dget  (x, y)    { var i=find(x.k, y); return (i.v==len(x.k)) ? NA : atx(x.v, i);}					// dict get at key
function lset  (x, y, z) { 																					// list set
	if (len(x) <= p(y)) { throw new Error("index error."); } 
	x.v[y.v]=z; 
}
function dset  (x, y, z) { var i=find(x.k, y).v; if(i==len(x.k)) { x.k.v.push(y); } x.v.v[i]=z; }			// dict set
function lower (x)       { return k(1, String.fromCharCode(x.v).toLowerCase().charCodeAt(0)); }				// lower case
function kmap  (x, f)    { return k(3, l(x).v.map(f)); }													// map to everything in the list and return a list
function kzip  (x, y, f) { return kmap(sl(x,y), function(z, i) { return f(z, y.v[i]); }); }					// zip 
function sl    (x, y)    { if (len(x) != len(y)) { throw new Error("length error."); } return x; }			// if x and y same length, return x
function n     (x)       { return (x.t==0||x.t==1) ? x : ct(x, 0); }										// parse to number, error out if not int or char
function l     (x)       { return ct(x, 3); }																// check if list
function d     (x)       { return ct(x, 4); }																// check if dict
function a     (x)       { if (x.t > 2) { throw new Error("domain error."); } return x; }					// check if atom(int or char)
function na    (x)       { return x.t === 0 && isNaN(x.v); }												// Check if x is NA
function stok(x) { var v=kl(krange(x.length, function(z) { return k(1,x.charCodeAt(z)); }).v);				// javascript string to k char list
	return  v;
}
function c(x)    {																							// don't change a thing, is it copy?
	var v=(x.t==3) ? k(x.t, x.v.slice(0)) : (x.t==4) ? md(c(x.k), c(x.v)) : x;
	return v;
}
function ct(n,t) { if (n.t!=t) throw new Error(TN[t]+" expected, found "+TN[n.t]+"."); return n; }			// check type match
function p(x) { if (n(x).v<0||x.v%1!=0) { throw new Error("positive int expected."); } return x.v; }		// positive int
function ktos(x, esc) {																						// toString
	if (x.t != 3) { x = enlist(x); }
	var h = x.v.some(function(v){ return (v.v<32||v.v>127)&v.v!=9&v.v!=10; });
	if (h) { return "0x"+x.v.map(h2).join(""); }															// if contains non-character, list hex bytes
...
function max   (x, y) { return na(x)?y:na(y)?x:k(0, Math.max(n(x).v, n(y).v)); }							// why check NA with max but not min?	
function min   (x, y) { return                 k(0, Math.min(n(x).v, n(y).v)); }
function less  (x, y) { return kb(a(x).v < a(y).v); }														// < in k
function more  (x, y) { return kb(a(x).v > a(y).v); }														// > in k
function equal (x, y) { return kb((x.v == y.v) || (na(x) && na(y))); }										
function join  (x, y) { 																					// x='.', y=['a', 'b', 'c']
	var v=l(y).v.reduce(function(z, y) { return cat(z, cat(x, y)); });
	return  v;
}				// ?
function ident    (x) { return x; }																			// where is it used?
function negate   (x) { return k(0, -n(x).v); }																
function first    (x) { return (x.t == 4) ? first(x.v) : (x.t != 3) ? x : len(x) ? x.v[0]:k(3,[]); }		// first element
function sqrt     (x) { return k(0, Math.sqrt(n(x).v)); }
function keys     (x) { return c(d(x).k); }																	// keys of a dict, that does c() do here??
function rev      (x) { return x.t==4?md(rev(x.k),rev(x.v)):x.t==3?k(3,c(l(x)).v.reverse()):x; }			
function asc      (x) { return grade(-1, x); }																// grade asc
function desc     (x) { return grade(1, x); }
function not      (x) { return equal(n(x), k0); }															// when does x==true?
function enlist   (x) { return k(3, [x]); }
function isnull   (x) { return max(match(x, NIL),match(x,k(11))); }											// null if NIL or k(11, undefined)
function count    (x) { return k(0, x.t == 4 ? len(x.v) : x.t == 3 ? len(x) : 1); }							
function floor    (x) { return x.t == 1 ? lower(x) : k(0, Math.floor(n(x).v)); }
function type     (x) { return k(0, kt[x.t]); }																// the type itself is a k number
function kfmt     (x) { 																					// parse js string to k char array
	var r=stok(format(x, 0, 1)); 
	var v = r.t == 3 ? r : enlist(r); 
	return v;
}						
function real     (x) { return krange(n(x).v, function() { return k(0, Math.random()); }); }				// return x random nums

function iota(x) {																							// bang operator
	if (x.t == 4) { return keys(x); }
	var i = krange(Math.abs(n(x).v), k.bind(null, 0)); 
	var v = x.v>=0 ? i : ar(plus)(x, i);
	return v;
}

function cat(x, y) {																						// concat x and y
	if (x.t==4&&y.t==4) { x=c(x); kmap(y.k, function(v) { dset(x,v,dget(y,v)); }); return x; };
	return k(3, (x.t==3?x.v:[x]).concat(y.t==3?y.v:[y]));
}

function keval(x, env) {																					// eval
	if (x.t == 5) { return x.env.d; }																		
	return x.t == 4 ? c(x.v) : x.t == 2 ? env.lookup(x, true) : run(parse(ktos(x)), env);					
}

function dfmt(x, y) {																						// parse to k string
	if ( x.t == 3           && y.t == 3) { 																	// ([2, 3, 4], [c97, c98, c99]) -> [c97, c98, c99]
		var v = kzip(x, y, dfmt); 
		return v;
	}
	if ( x.t == 3           && y.t != 3) { 
		var v = kmap(x, function(z) { return dfmt(z, y); }); 
		return v;
	}
	if ((x.t == 2 || !s(y)) && y.t == 3) { 	
		var v = kmap(y, function(z) { return dfmt(x, z); }); 
		return v;
	}
	if (x.t == 2) { 																						
		var v = {b: k(0,y.v&1), i: k(0,y.v|0), f: k(0,y.v), c: k(1,y.v)}[x.v]; 
		return v;
	}
	if (y.t == 1) { 
		console.log('y is number, return y');
		return y; 
	} 
...
function except(x, y) {																						// difference between except and drop?
	y = y.t == 3 ? y : enlist(y);
	return k(3, (x.t == 3 ? x : iota(x)).v.filter(function(z) { return na(pfind(y, z)); }));
}
function ddrop(x, y) { 																						// drop key list x from dict y
	var k = except(d(y).k, x); 
	var v = md(k, atx(y, k)); 
	return v;
}							
function drop(x, y) {																						// drop index list x from y
	if (y.t == 4) { 
		var v = md(drop(x, y.k), drop(x, y.v)); 
		return v;
	}
	var v = (y.t != 3 || len(y) < 1) ? y : k(3, n(x).v<0 ? y.v.slice(0,x.v) : y.v.slice(x.v));
	return v;
}
function take(x, y, env) {																					// use y to fill a shape defined by x
	if (x.t == 5 || x.t == 8 || x.t == 9) {																	// TODO: don't get how functions work
		var k = where(each(x, y, env), env); var v = atx(y, k);
		var v = y.t == 4 ? md(k, v) : v;
		return v;
	}
	if (y.t == 4) { 
		return md(take(x, y.k, env), take(x, y.v, env)); 
	}
...
function match(x, y) {																						// Distribute if list or dict
...
function find(x, y) { y=x.v.findIndex(function(z){return match(z,y).v}); return k(0,y>=0?y:len(x)) }		// find y in x
function pfind(x, y) { y=x.v.findIndex(function(z){return equal(z,y).v}); return y>=0?k(0,y):NA }			// find atom y in x
function pisnull(x) { return kb(match(x, NIL).v || match(x, k(11)).v || na(x)); }							// isnull returns a k binary

function cut(x, y) {																						// 1?
	console.log('Function cut input');
	console.log(x);
	console.log(y);
	var v = kzip(x, cat(drop(k1,x),count(y)), function(a, b) { // {x{x@y+!z-y}[y]'1_x,#y} ?
...
function split  (x, y) { return (x.t != 1) ? unpack(x, y) : call(splitimpl, k(3, [x,y])); }					// 9?
function unpack (x, y) { return call(unpackimpl, k(3, [x,y])); }											// 10? 
function pack   (x, y) { return (x.t == 1) ? join(x, y) : call(packimpl, k(3, [x,y])); }					// 11?
function kwindow(x, y) { return call(winimpl, k(3, [x,y])); }												// 12?
function splice(xyz)   { return call(spliceimpl, k(3, xyz)); }												// 13?
function imat(x)       { var i = iota(x); return kmap(i, function(z) { return ar(equal)(z, i); }); }		// 14?
function odometer(x)   { return call(odoimpl, enlist(x)); }													// 15?
...
function am(f) { 																									// create an atomic monad, apply down to atoms
...
function applyd(verb, x, y, env) {																					// apply dyad
	if (verb.t == 5) { return call(verb, k(3,[x,y]), env); }														// call(v, [x]) and call(v, [x, y])?
...
var verbs = {
	//     a          l           a-a         l-a         a-l         l-l         triad    tetrad					// atom and list??
...
function applyverb(node, args, env) {																					// ?
	...
	if (node.t == 9) { return applyadverb(node, node.verb, args, env); }												// why node is adverb?
	var left  = args.length == 2 ? args[0] : node.l ? run(node.l, env) : null;											// run lhs first
	var right = args.length == 2 ? args[1] : args[0];
	if (!right) { return { t:node.t, v:node.v, curry:[left,k(11)] }; }													// what is returned here?
	var r = null; 
	var v = verbs[node.forcemonad ? node.v[0] : node.v];
	if (!v) {}
...
function valence(node, env) {																							// num of arg expected
	if (node.t == 5) {
	...
	if (node.t != 8)       { return 0; }																				// not verb, valence is 0
...
function applyadverb(node, verb, args, env) {
	if (verb.t == 7) { verb = run(verb, env); }																			// when is verb a nameref
	var r = null; var v = valence(verb, env);																			
	if (v > 2)                 { return adverbs[node.v][4](verb, args, env); }											
	if (v == 0 && verb.t != 5) { return applyverb(k(8,node.v), [verb, args[1]], env); }									// apply adv to v and return
	if (v == 0 && verb.t == 5) { v = 1; }																				// ?
	...
	return args[0] ? r(verb, args[0], args[1], env) : r(verb, args[1], env);											// when is 1 arg and when 2?
}
...
function atx(x, y, env) {																			// index y from x

	var v = x.t == 2 ? atx(env.lookup(x), y, env) : y.t == 11 ? x : 								// y is Nil then return x
	       x.t == 3 && y.t == 4 ? md(y.k, atx(x, y.v, env)) :
	       x.t == 8 || x.t == 9 ? applym(x, y, env) : 												// if x is verb and y is adverb, apply?
	       (x.t == 3 || x.t == 4) && y.t == 3 ? kmap(y, function(z) { return atx(x, z); }) :
	       x.t == 3 ? (y.t > 1 || y.v < 0 || y.v >= len(x) || y.v%1 != 0) ? NA : x.v[y.v] : 		// no negative index for list, but char index makes char taken as int
	       x.t == 4 ? dget(x, y) : call(x, enlist(y), env);											// if nothing satisfies, call(x, enlist(y))?
	return v;
}
...
function call(x, y, env) {
	...
	if (x.t == 3 || x.t == 4) { return y.t == 3 ? atdepth(x, y, 0, env) : atx(x, y, env); }			// only 1 step deeper in y?
	...
	if (y.t == 4) { var e=new Environment(null); e.d=y; x.env=e; return x; }						// x.t=5 and y.t=4, why return?
	if (y.t != 3) { y = enlist(y); }
...
function run(node, env) {
	if (node instanceof Array) { 
		var res = node.reduce(function(_,x) { return run(x, env); }, null); 
		return res;
	}
	if (node.sticky) { 
		return node; }
...
function amend4(args, env) { return mend(args, env, amendm, amendd); }													// verb @ TODO
function dmend4(args, env) { return mend(args, env, dmend, dmend); }													// verb . TODO

function mend(args, env, monadic, dyadic) {
	var ds = args[0], i = args[1], f = args[2], y = args[3];
	(y?dyadic:monadic)(ds.t == 2 ? env.lookup(ds,true) : ds, i, y, f, env); return ds;									// hv args[3] -> dyad
}
...
var NUMBER  = /^(-?0w|0N|-?\d+\.\d*|-?\d*\.?\d+)/;																				// what are 0w and 0N?
...
function at(regex)      { return regex.test(text); }																					// true or false
function matches(regex) { return at(regex) ? expect(regex) : false; }
function expect(regex) {																												// update funcdepth on the way of matching
...
function findNames(node, names) {																										// what are the names?
...
function indexedassign(node, indexer) {																									
	...
	//t[x]::z  ->  ..[`t;x;{y};z]   t[x]:z  ->  t:.[t;x;{y};z]																			// k does this too?
...
function applycallright(node) {
	var node_copy=Object.assign({}, node);
	var openb=false;																						// TODO
	while (matches(OPEN_B)) {
		var args = parseList(CLOSE_B); node = asVerb(".", node, k(3, args.length ? args : [NIL]));						// asVerb('.', node, [args]) what does this do?
		openb=true;																													// t=7 will return as it is
	}
	if(openb){
		console.log('Before applycallright: ');
		console.log(JSON.stringify(node_copy));
		console.log('Finished callright: ');
...
function applyindexright(node) {	
	...
	while (matches(OPEN_B)) { node = asVerb(".", node, k(3, parseList(CLOSE_B))); openb=true; }
	if(openb){
		console.log('Indexright input: ');
		console.log(JSON.stringify(node_copy));
		console.log("Finished indexright: ");
		console.log(JSON.stringify(node));
	}
	return node;
}
...
function parseNoun() {
	...
	if (at(STRING)) {
		...
		for(var z=0;z<EC.length;z++) { str=str.split(EC[z][1]).join(EC[z][0]); }											// un-escape
